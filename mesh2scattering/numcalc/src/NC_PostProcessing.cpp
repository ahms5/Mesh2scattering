...
// local functions
void NC_ComputeTvecSLFMM(ofstream&,bool,const Matrix<Complex>&,const Matrix<Complex>&);
...
)
{
  ...
  if( evalonly ) {
    /* as we did not calculate any values on the surface, we will have
       to read them from the resultfiles, sounds easy, can become quite
       tricky when there are more then 2 groups
    */
    int elems_set = 0;
    int igrp,ngrp,iel,dummy;
    char dummybuffer[100]; // if the first line becomes larger, change here
                           // *and* below in the fgets statements
    double vreal, vimag;
    sprintf(Dfname_3d, "be.out/be.%d/pBoundary",  currentFrequency_ + 1);
    Lu_pBoundary = fopen(Dfname_3d, "r");
    ...
    Lu_vBoundary = fopen(Dfname_3d, "r");
    if(!Lu_vBoundary) NC_Error_Exit_1(NCout,  "Can not open the file ", Dfname_3d);
    
    
    // get rid of the first two lines
    fgets(dummybuffer,100,Lu_pBoundary);
    fgets(dummybuffer,100,Lu_pBoundary);
    
    fgets(dummybuffer,100,Lu_vBoundary);
    fgets(dummybuffer,100,Lu_vBoundary);
    //	  fscanf(Lu_pBoundary, "%d", &NumElementGroups);
    /* now it gets tricky, we may more then one BE group, which means 
       that there is a chance that we have to rerun the loop over all 
       elemets */
    ngrp = 0;
    do {
      if( ngrp == 0 ) {
	/* okay, this means, we have not found all BEM values yet, which means
	   the file contains an additional group */
	fscanf(Lu_pBoundary, "%d %d", &igrp, &ngrp);
	fgets(dummybuffer,100,Lu_vBoundary);
      }
      
      for(iel = 0; iel < numElements_ ; iel++) {
	if( listElementProperty[iel] == 2 || igrp != listElementsElementGroup[iel]) continue; // eval elem or wrong group
	if(ngrp > 0) {
	  fscanf(Lu_pBoundary,"%d %lf %lf", &dummy, &vreal, &vimag);
	  zVpotele(iel,0).set(vreal,vimag);
	  zVpotele(iel,0).div_i(rpfact_);
	  fscanf(Lu_vBoundary,"%d %lf %lf", &dummy, &vreal, &vimag);
	  zVeloele(iel,0).set(vreal,vimag);
	  elems_set++;
	  ngrp--;
	}
	if( elems_set == numElementsOfBoundaryMesh_ ) {
	  // everything is set, the rest can only be eval
	  break;
	}
      } // elem loop
    } 
    while ( elems_set < numElementsOfBoundaryMesh_ );
    fclose(Lu_pBoundary);
    fclose(Lu_vBoundary);
  } // if evalonly
  ...
  if(IfTvc) {
    ...
    case 3: // DMLFMBEM
      if (evalonly) {
	// we only need the first cluster level, no need to waste memory
	// on the rest
	zT_vc = new Complex[numOriginalReflectedClusters_*numIntegrationPointsUnitSphere_];
	NC_ComputeTvecSLFMM(NCout, evalonly, zVpotele, zVeloele);
      }
...
(
 ofstream& NCout,
 bool evalonly,  // only calulcate the values at the eval nodes
 const Matrix<Complex>& zVpotele, // velcitypot. at BEM elems
 const Matrix<Complex>& zVeloele  // part veloc  at BEM elems
 )
{
  ...
  for(i=0; i<numOriginalReflectedClusters_; i++) {
	...
	// here it gets tricky in the evalonly case, because
	// NC_ComputeElementData wants zrhs in the 1 flag case,
	// which is not available
	if( evalonly ) {
	  NC_ComputeElementData(jel, 0, inodj, iveloj,
				ifadmij, jdofaddre, centerj, norvecj, zpotej, zveloj, crdelj);
	  zpotej[0] = zVpotele(jel,0);
	  for (k = 0; k < inodj; k++ ) 
	    zveloj[k] = zVeloele(jel,k);
	}
	else
 ...
 )
{
  ...
  Vector<double> csip(NNODPE), etap(NNODPE), dwk0(NDIM), Pm(numExpansionTerms_), crdip(NDIM);
